...
5. To verify the privacy claims:
   ...
   - All other functionality (scoring, charting, fallback) is on-device.
...
- [x] **Manifest icons** — 16, 32, 48, 96, 128 px PNG present in `public/icon/`.
- [x] **CSP** — `script-src 'self' 'wasm-unsafe-eval'; object-src 'self'`, verified in `wxt.config.ts`.
- [x] **`homepage_url`** — `https://github.com/tahamtandariush/smartdeal-hunter`.
- [x] **`author.email`** — `tahamtandariush@gmail.com`.
- [ ] **Bundle size** — Verify zip ≤ 2.5 MB.
- [ ] **5 screenshots @ 1280×800 PNG** — (User task).
- [ ] **Privacy policy hosted at a public URL** — (User task).
