...
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | -------- |
| Limited Use: collect only data required for single purpose | Genome stored locally only; no remote transmission by default; Deep Check only after explicit opt-in | Code audit + network proxy test (zero default external calls) | Critical |
...
| Disable Deep Check / external API | Options > Privacy          | Required           | toggle off → all external calls blocked           |
| View local audit trail            | Options > Privacy          | Required           | inspect last 50 Deep Check outcomes locally       |
| Schedule secure wipe              | Options > Privacy          | Required           | create local alarm → wipe unless action in flight |
